"""
Project 2 – Q2
Streamlit App with Open-Source LLM (Ollama)
...
3) Start the model (as used in my workflow):
   ollama run llama3.2

4) Run the Streamlit app:
   streamlit run Project2_Q2.py

This file implements:
• Question 2 – automatic extraction of all abbreviations.
• Also includes document loading and optional LLM-based Q&A.
"""
...
st.write("You can ask a question about a document and/or extract abbreviations from it.")

# Optional user question for the LLM
question = st.text_input("Enter your question (optional):")

st.caption(
    "If you only want abbreviation extraction from the uploaded article, "
    "just attach the file and click 'Ask' without typing a question."
)
...
if st.button("Ask"):
    ...
    if context_text:
        ...
        st.write("No document was uploaded, so abbreviations cannot be extracted.")

    # Question-answering with ChatOllama
    if question.strip():
...
Answer the question clearly and concisely. Use the document when it is relevant.
If the document is empty or not helpful, answer from general knowledge.
"""
        qa_response = llm.invoke(qa_prompt)

        st.subheader("AI Answer:")
        st.write(qa_response.content)
